...
class XhtmlResponser extends Responser {

	public function render() {

		$view = new View($this->layout);
		$view->setPath( DP_MODULES_DIR . DaoPHP::getInstance()->getModule() . DS . 'layout' . DS );
		
		$view->setViewSuffix( $this->layoutSuffix );

		$view->assign('title', $this->getTitle() );
		$view->assign('keywords', $this->getKeywords() );
		$view->assign('description', $this->getDescription() ) ;

		$headerLines = '' ;
		if( count( $this->getHeaderLines() ) ) {
			$headerLines = implode("\n", $this->getHeaderLines());
		}

		$view->assign('headers', $headerLines );
		$view->assign('content', $this->getContent() );
